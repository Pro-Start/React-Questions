...
import { React_Nav_Items } from "../routes/ProjectRoutes";

export const ProjectNavbarReact = () => {

    const [active, setActive] = useState(React_Nav_Items[0].name)
    ...
    useEffect(() => {
        // change page name
        document.title = "React-Questions"
        // change page icon
        const link: any = document.querySelector("link[rel*='icon']") || document.createElement('link');
        link.type = 'image/x-icon';
        link.rel = 'shortcut icon';
        link.href = 'https://raw.githubusercontent.com/dependabot-pr/Static-Files/main/Assets/Logo/Technologies/React.svg';
            ...
            <div className={`sticky top-0 bg-white dark:bg-gray-800 z-50 items-center ${show ? '' : ' shadow-sm'} `}>
                <nav className={`container flex flex-row justify-between dark:bg-gray-800 mx-auto p-3 py-4 font-sans ${show ? 'bg-white' : 'bg-white'} `}>
                    <Link
                        ...
                        onClick={() =>
                            ButtonPress(React_Nav_Items[0].name)}
                    ...
                    <div className={`flex-row gap-7 items-center hidden lg:flex ${show ? '' : ''}`}>
                        {
                            React_Nav_Items.map((item, index) => {
                                return (
                                    <Link
                                        onClick={() =>
